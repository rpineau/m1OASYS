...
{
    str = "m1 OASYS X2 plugin by Rodolphe Pineau";
}
...
int X2Dome::dapiGetAzEl(double* pdAz, double* pdEl)
{
    ...
    *pdAz = m1OASYS.getCurrentAz();
    *pdEl = m1OASYS.getCurrentEl();
    return SB_OK;
}
...
int X2Dome::dapiIsGotoComplete(bool* pbComplete)
{
    ...
    err = m1OASYS.isGoToComplete(*pbComplete);
    if(err)
        return ERR_CMDFAILED;
    return SB_OK;
}
...
int X2Dome::dapiIsOpenComplete(bool* pbComplete)
{
    ...
    if(err) {
        snprintf(mLogBuffer,ND_LOG_BUFFER_SIZE,"[X2Dome::dapiIsOpenComplete] ERROR while checking for opened status of th shutter");
        m_pLogger->out(mLogBuffer);
...
int	X2Dome::dapiIsCloseComplete(bool* pbComplete)
{
    ...
    if(err) {
        snprintf(mLogBuffer,ND_LOG_BUFFER_SIZE,"[X2Dome::dapiIsCloseComplete] ERROR while checking for closed status of th shutter");
        m_pLogger->out(mLogBuffer);
...
int X2Dome::dapiIsParkComplete(bool* pbComplete)
{
    ...
    err = m1OASYS.isParkComplete(*pbComplete);
    if(err)
        return ERR_CMDFAILED;
...
int X2Dome::dapiIsUnparkComplete(bool* pbComplete)
{
    ...
    err = m1OASYS.isUnparkComplete(*pbComplete);
    if(err)
        return ERR_CMDFAILED;
...
int X2Dome::dapiIsFindHomeComplete(bool* pbComplete)
{
    ...
    err = m1OASYS.isFindHomeComplete(*pbComplete);
    if(err)
        return ERR_CMDFAILED;
